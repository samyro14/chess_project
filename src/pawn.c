...
void promotion(GameState* state, int x, int y){
    // Promovare la regină
    Piece promoted_piece = {QUEEN, state->board[y][x].color};
    state->board[y][x] = promoted_piece;
...
void move_pawn(GameState* state, int from_x, int from_y, int to_x, int to_y){
    ...
    if(is_valid_pawn(state, from_x, from_y, to_x, to_y) > 0){
        ...
        int promotion_row = state->board[to_y][to_x].color == WHITE ? 0 : 7;
        if (to_y == promotion_row) {
            promotion(state, to_x, to_y);
        }
